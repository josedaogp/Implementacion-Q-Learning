...
public class Agent2 extends AbstractPlayer {
	...
	public static final double k =2000;
    ...
    public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer)  {
		...
		if (aleatorio < epsilon) { //si el numero aleatorio es menor que epsilon, se ejecuta una accion aleatoria
			...
			do{
				...
				if((s.getAbajo()==-1 && accion == 2) || (s.getArriba()==-1 && accion == 3)) repetir = true;
			}while(repetir);
			
			//Devolver accion aleatoria. Se puede generar un entero entre 1 y 4 y al final del metodo act devolverlo
		}
		...
		Estado2 sPrima = new Estado2(Futuro);
		...
		else if (Entorno2.celdaConGasolina(Entorno2.getPosAvatar(Futuro), Futuro)) {
			
			recompensa ++;
		}
		else if(sPrima.getGasolina() == 1 || sPrima.getGasolina() == 2) {
			recompensa += 0.1;
		}
		
		else recompensa += 0.05;
		...
		/*Entorno4 e = new Entorno4(Futuro);
		e.verMapa();*/
		/*System.out.println();
