...
public class AgentEstado extends AbstractPlayer {
	...
	public static final float gamma = 0.65f;
	public static final double k = 1500;
	...
	public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
		...
		if (Futuro.getGameWinner() == Types.WINNER.PLAYER_LOSES) {
			recompensa -= 2;
		} else if (Futuro.getGameWinner() == Types.WINNER.PLAYER_WINS) {
			recompensa += 1;
		} else {
			if (Entorno.celdaConGasolina(Entorno.getPosAvatar(Futuro), Futuro)) {
				recompensa += 1;
			}
			if (s.gasolinaMasCerca(sPrima) && sPrima.getDistanciaObstaculo()>=2) {
				recompensa +=0.45;
			}
			if (sPrima.getLibre() == 2) { // Recompensa si esta en la columna de la celda libre
				recompensa += 0.1;
			}

			if (sPrima.getGasolina() == 1) { // Recompensa si esta en la columna de la celda de gasolina
				recompensa += 0.2;
			}
		}
		 ...
		 * e.verMapa(); Entorno f = new Entorno(Futuro);
